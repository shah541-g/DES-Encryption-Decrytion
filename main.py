...
class DESApp:
    def __init__(self, window):
        self.window = window
        self.window.title("Encryption and Decryption (DES)")
        heading = tk.Label(window, bg="#27A58C",text="DES Encryption and Decryption", font=("Times New Roman", 20, "bold"))
        heading.pack(pady=20)
        self.window.config(padx=20,pady=20,bg="#27A58C")
        ...
        self.upload_button = tk.Button(window, text="Upload PDF", command=self.upload_file,bg="#1B8314",fg="white",width=20,height=2)
        self.upload_button.pack(pady=10)

        self.text_area = ScrolledText(window, wrap="word", height=15)
        self.text_area.pack(fill="both", expand=True, padx=10, pady=10)

        self.action_button = tk.Button(window,text="Encrypt/Decrypt and Save",bg="#092768",fg="white",command=self.process_text,width=30,height=2)
